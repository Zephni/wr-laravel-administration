@foreach($WRLAHelper::getNavigationItems() as $navigationItem)
    ...
    @else
        <div x-data="{
            ...
            <div class="relative flex items-stretch justify-between h-fit w-full whitespace-nowrap select-none font-bold bg-slate-700 border-b border-slate-400">
                ...
                @if(!empty($navigationItem->route))
                    <a href="{{ $navigationItem->getUrl() }}"
                        ...
                        class="flex-1 h-full px-6 text-slate-200-6 pt-2 pb-1 text-start text-slate-200 hover:text-primary-500 hover:bg-slate-800 whitespace-nowrap">
                ...
                {{-- Dropdown arrow --}}
                <div @click="dropdownOpen = !dropdownOpen" x-bind:class="{'': !dropdownOpen, '': dropdownOpen}" class="absolute right-0 flex justify-center items-center w-8 min-w-8 min-h-full text-slate-200-slate-800 text-slate-300 dark:text-slate-500 cursor-pointer hover:text-primary-500 border-l border-gray-400 dark:border-gray-700">
            ...
            <div x-show="dropdownOpen" class="w-full" x-transition x-transition:enter.duration.300ms x-transition:leave.duration.300ms>
                @foreach($navigationItem->children as $child)
                    <a
                        ...
                        href="{{ $child->getUrl() }}" class="@if($child->getUrl() == url()->current()) !text-primary-500 @endif flex items-center justify-start w-full pl-9 pr-6 pt-1 text-slate-200 pb-0 font-bold bg-slate-700 hover:bg-slate-800 hover:!text-primary-500 dark:hover:!text-primary-500 border-b border-slate-400">
