...
<div
    x-data="{ leftPanelOpen: true, leftPanelWidth: $persist(360), dragging: false, startX: 0, minW: 0, maxW: 0 }"
    x-bind:style="'width: ' + leftPanelWidth + 'px;'"
    :class="(leftPanelOpen ? 'min-w-44 max-w-[33%] ' : 'min-w-0 max-w-0 ') + (!dragging ? 'transition-all' : '')"
    id="left-panel"
    class="relative flex flex-col justify-start items-start h-full border-r-2 border-slate-300 dark:border-slate-950 bg-slate-100 dark:bg-slate-850 shadow-xl shadow-slate-500 dark:shadow-slate-950 z-10">
    ...
    <button
        ...
        class="absolute z-10 top-0 pt-1 w-5 opacity-60 text-sm flex justify-center items-center bg-slate-50 dark:bg-slate-800 text-slate-500 dark:text-slate-400 shadow-md shadow-slate-400 dark:shadow-slate-900 border-l border-b border-slate-300 dark:border-slate-400 cursor-pointer rounded-bl"
        <i x-bind:class="{'fas fa-chevron-left': leftPanelOpen, 'fas fa-chevron-right': !leftPanelOpen}"></i>
    </button>

    {{-- Resize bar --}}
    <div class="absolute top-0 -right-1 h-full w-[4px] bg-slate-300 dark:bg-slate-800 border-r border-slate-200 dark:border-slate-850 cursor-ew-resize"
        @mousedown="$event.preventDefault(); startX = $event.clientX; dragging = true;"
